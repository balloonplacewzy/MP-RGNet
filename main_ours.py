...
RESTORE_OUTPUT_KEYS = (
    "p_impute", "impute", "imputed", "imputation",
    "p_fuse", "P_fuse", "pv_fuse",
    "p_corr", "P_corr", "pv_corr",
    "reconstruction", "recon", "pv_recon", "x_hat", "pv_hat",
    "p_base", "P_base",
)


def extract_named_tensor(model_output, keys):
    ...
    if isinstance(model_output, dict):
        for key in keys:
            ...
            if torch.is_tensor(value):
                return value, key
        for nested_key in ("extra", "extras", "aux", "auxiliary", "outputs"):
...
def normalize_history_tensor(tensor: torch.Tensor, batch, name: str = "history_tensor"):

    if not torch.is_tensor(tensor):
        raise TypeError(f"{name} must be a torch.Tensor.")
    if tensor.dim() != 3:
        raise ValueError(f"{name} should have shape [B, L, N] or [B, N, L], got {tuple(tensor.shape)}.")
    target = batch["pv_clean"]
    b, l, n = target.shape
    if tensor.shape == target.shape:
        return tensor
    if tensor.shape[0] == b and tensor.shape[1] == n and tensor.shape[2] == l:
        return tensor.transpose(1, 2)
    raise ValueError(
        f"{name} shape {tuple(tensor.shape)} is incompatible with pv_clean shape {tuple(target.shape)}. "
        f"Expected [B, {l}, {n}] or [B, {n}, {l}]."
    )


def weighted_impute_loss(p_restore: torch.Tensor, batch, args):

    p_restore = normalize_history_tensor(p_restore, batch, name="p_restore")
    p_clean = batch["pv_clean"]
    obs_mask = batch["obs_mask"]
    day_mask = batch.get("day_mask", None)
    abs_err = torch.abs(p_restore - p_clean)
    weight = torch.ones_like(abs_err)
    w_miss = float(getattr(args, "w_miss", 3.0))
    w_day = float(getattr(args, "w_day", 1.0))
    w_obs_day = float(getattr(args, "w_obs_day", 0.0))
    weight = weight + w_miss * (1.0 - obs_mask)
    if day_mask is not None:
        weight = weight + w_day * day_mask
        weight = weight + w_obs_day * obs_mask * day_mask
    return (weight * abs_err).sum() / (weight.sum() + 1e-6)


def temporal_smooth_loss(p_restore: torch.Tensor, batch, args):

    p_restore = normalize_history_tensor(p_restore, batch, name="p_restore")
    if p_restore.shape[1] <= 1:
        return torch.zeros((), device=p_restore.device, dtype=p_restore.dtype)
    diff = p_restore[:, 1:, :] - p_restore[:, :-1, :]
    return torch.mean(torch.abs(diff))


def compute_ours_loss(model_output, batch, criterion, args):

    pred, _ = parse_model_output(model_output)
    pred_loss = criterion(pred, batch["y"])
    p_restore, restore_key = extract_named_tensor(model_output, RESTORE_OUTPUT_KEYS)
    has_restore = p_restore is not None
    zero = torch.zeros((), device=pred_loss.device, dtype=pred_loss.dtype)
    impute_loss = zero
    smooth_loss = zero
    if has_restore:
        p_restore = normalize_history_tensor(p_restore, batch, name=restore_key or "p_restore")
        impute_loss = weighted_impute_loss(p_restore, batch, args)
        smooth_loss = temporal_smooth_loss(p_restore, batch, args)
    stage = str(getattr(args, "stage", "joint")).lower()
    lambda_impute = float(getattr(args, "lambda_impute", 0.0))
    lambda_smooth = float(getattr(args, "lambda_smooth", 0.0))
    if stage == "restore":
        if not has_restore:
            raise KeyError("stage='restore' requires model output {'p_impute': p_restore}.")
        total_loss = impute_loss + lambda_smooth * smooth_loss
    elif stage == "forecast":
        total_loss = pred_loss
    elif stage == "joint":
        total_loss = pred_loss
        if lambda_impute > 0.0:
            if not has_restore:
                raise KeyError("lambda_impute > 0 requires p_impute/pv_hat/p_fuse in model output.")
            total_loss = total_loss + lambda_impute * impute_loss
        if lambda_smooth > 0.0:
            if not has_restore:
                raise KeyError("lambda_smooth > 0 requires p_impute/pv_hat/p_fuse in model output.")
            total_loss = total_loss + lambda_smooth * smooth_loss
    else:
        raise ValueError("stage should be one of: restore, forecast, joint.")
    logs = {
        "total_loss": total_loss.detach(),
        "pred_loss": pred_loss.detach(),
        "impute_loss": impute_loss.detach(),
        "smooth_loss": smooth_loss.detach(),
        "has_restore": float(has_restore),
    }
    return total_loss, pred, logs


def init_loss_meters():
    return {"total_loss": [], "pred_loss": [], "impute_loss": [], "smooth_loss": [], "has_restore": []}


def update_loss_meters(meters, logs):
    for key in meters:
        ...
        meters[key].append(float(value))


def average_loss_meters(meters):
    return {key: float(np.mean(values)) if len(values) > 0 else np.nan for key, values in meters.items()}


def resolve_monitor_name(args):
    metric = str(getattr(args, "early_stop_metric", "auto")).lower()
    stage = str(getattr(args, "stage", "joint")).lower()
    if metric == "auto":
        metric = "impute" if stage == "restore" else "pred"
    mapping = {
        "total": "total_loss",
        "pred": "pred_loss",
        "forecast": "pred_loss",
        ...
        "impute": "impute_loss",
        "restore": "impute_loss",
        "restoration": "impute_loss",
        "smooth": "smooth_loss",
    }
    if metric not in mapping:
        raise ValueError("early_stop_metric should be one of: auto, total, pred, impute, smooth.")
    ...
    for raw_batch in train_loader:
        ...
        if use_amp:
            with torch.cuda.amp.autocast():
                model_output = call_model(model, batch, args, pass_batch)
                loss, _, logs = compute_ours_loss(model_output, batch, criterion, args)
        ...
        update_loss_meters(meters, logs)

    return average_loss_meters(meters)
...
def validate_one_epoch(model, val_loader, criterion, device, args, pass_batch):
    ...
    for raw_batch in val_loader:
        batch = move_batch_to_device(raw_batch, device)
        model_output = call_model(model, batch, args, pass_batch)
        _, _, logs = compute_ours_loss(model_output, batch, criterion, args)
        update_loss_meters(meters, logs)
...
def save_metrics_csv(metrics, timing_info, args, dataset_name, save_path, total_params, trainable_params, meta):
    ...
    row = {
        ...
        "w_miss": float(getattr(args, "w_miss", 0.0)),
        "w_day": float(getattr(args, "w_day", 0.0)),
        "w_obs_day": float(getattr(args, "w_obs_day", 0.0)),
...
def main(cli_args):
    saved_cli = {
        ...
        "stage": getattr(cli_args, "stage", None),
        "early_stop_metric": getattr(cli_args, "early_stop_metric", None),
        "w_miss": getattr(cli_args, "w_miss", None),
        "w_day": getattr(cli_args, "w_day", None),
        "w_obs_day": getattr(cli_args, "w_obs_day", None),
    }
    ...
    args.stage = str(getattr(args, "stage", "joint") or "joint").lower()
    args.early_stop_metric = str(getattr(args, "early_stop_metric", "auto") or "auto").lower()
    if getattr(args, "lambda_impute", None) is None:
        args.lambda_impute = 0.1 if args.stage == "joint" else 0.0
    else:
        args.lambda_impute = float(args.lambda_impute)
    ...
    args.w_miss = float(getattr(args, "w_miss", 3.0) if getattr(args, "w_miss", None) is not None else 3.0)
    args.w_day = float(getattr(args, "w_day", 1.0) if getattr(args, "w_day", None) is not None else 1.0)
    args.w_obs_day = float(getattr(args, "w_obs_day", 0.0) if getattr(args, "w_obs_day", None) is not None else 0.0)

    args.impute_method = resolve_impute_method(args.scenario_name, args.impute_method, args.model_type)
    if args.model_type is None:
        args.model_type = "ours"
    if args.model_type != "ours":
        raise ValueError("main_ours.py is intended for model_type='ours'. Use main_comparison.py for baselines.")
    if args.impute_method != "zero":
    ...
    run_identifier = (
        f"{args.model_name}_type-{args.model_type}_stage-{args.stage}_scenario-{args.scenario_name}_imp-{args.impute_method}_"
    ...
    print("=" * 100)
    print("[PV QUALITY FORECASTING] STARTING CUSTOM OURS TRAINING")
    print("=" * 100)
    print(f"[Run] Model Name     : {args.model_name}")
    print(f"[Run] Model Type     : {args.model_type}")
    print(f"[Run] Stage          : {args.stage}")
    ...
    print(f"[Run] Restore weights : w_miss={args.w_miss}, w_day={args.w_day}, w_obs_day={args.w_obs_day}")
    print(f"[Run] Early stop      : {args.early_stop_metric}")
    ...
    if args.model_type == "ours":
        ...
        args.aux_per_site = int(meta.get("aux_per_site", 7))
        args.num_sites = int(meta["target_dim"])
    ...
    print("\n-----------------------------------Start Training----------------------------------")
    monitor_name = resolve_monitor_name(args)
    print(f"[Training] monitor={monitor_name}")
    ...
    for epoch in range(args.epochs):
        ...
        val_logs = validate_one_epoch(model, val_loader, criterion, device, args, pass_batch)

        if monitor_name not in val_logs or np.isnan(val_logs[monitor_name]):
            raise RuntimeError(f"Monitor metric {monitor_name} is unavailable in validation logs: {val_logs}")
        monitor_loss = float(val_logs[monitor_name])
        ...
        print(
            ...
            f"Train Total: {train_logs['total_loss']:.6f} | Train Pred: {train_logs['pred_loss']:.6f} | "
            f"Train Imp: {train_logs['impute_loss']:.6f} | "
            f"Val Total: {val_logs['total_loss']:.6f} | Val Pred: {val_logs['pred_loss']:.6f} | "
            f"Val Imp: {val_logs['impute_loss']:.6f} | Monitor({monitor_name}): {monitor_loss:.6f} | "
            f"LR: {current_lr:.6e}"
        )
    ...
    print("\n" + "=" * 100)
    print("[PV QUALITY FORECASTING] CUSTOM OURS TRAINING COMPLETED")
    print("=" * 100)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--early_stop_metric", type=str, default="auto",
                        choices=["auto", "total", "pred", "forecast", "prediction", "impute", "restore", "restoration", "smooth"],
                        help="Validation metric for scheduler/early stopping. auto: impute for restore, pred otherwise.")
    ...
    parser.add_argument("--w_miss", type=float, default=None, help="Extra weight for originally missing PV points.")
    parser.add_argument("--w_day", type=float, default=None, help="Extra weight for daytime PV points.")
    parser.add_argument("--w_obs_day", type=float, default=None, help="Extra weight for observed daytime PV points.")


    parser.add_argument("--model_name", type=str, default="newmodel", help="Model name under ./models and config name.")
    parser.add_argument("--model_type", type=str, default="ours", choices=["baseline", "ours"])
    parser.add_argument("--stage", type=str, default="joint", choices=["restore", "forecast", "joint"],
                        help="Training stage for custom models: restore, forecast, or joint.")
